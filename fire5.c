...
int signal_handler(int signo) {
  ...
  if (signo == SIGUSR1) {
    ...
    //read cooling1 parameter
    f=fopen("/tmp/lpd8806-fire-cooling1","r");
    if (f == NULL) { printf("fopen error %d\n", errno); return 1; }
    fgets(line,16,f);
    value = atoi(line);
    if ( (value > 0) || (value < 300) ) { cooling1 = value; }
    x1=((cooling1*10)/leds) + 2;
    printf("cooling1 set to %d\n", cooling1);
    fclose(f);

    //read cooling2 parameter
    f=fopen("/tmp/lpd8806-fire-cooling2","r");
    if (f == NULL) { printf("fopen error %d\n", errno); return 1; }
    fgets(line,16,f);
    value = atoi(line);
    if ( (value > 0) || (value < 300) ) { cooling2 = value; }
    x2=((cooling2*10)/leds) + 2;
    printf("cooling2 set to %d\n", cooling2);
    fclose(f);
    
    
    //read sparking parameter
    f=fopen("/tmp/lpd8806-fire-sparking1","r");
    if (f == NULL) { printf("fopen error %d\n", errno); return 1; }
    fgets(line,16,f);
    value = atoi(line);
    if ( (value > 0) || (value < 300) ) { sparking1 = value; }
    printf("sparking1 set to %d\n", sparking1);
    fclose(f);

    //read sparking parameter
    f=fopen("/tmp/lpd8806-fire-sparking2","r");
    if (f == NULL) { printf("fopen error %d\n", errno); return 1; }
    fgets(line,16,f);
    value = atoi(line);
    if ( (value > 0) || (value < 300) ) { sparking2 = value; }
    printf("sparking2 set to %d\n", sparking2);
    fclose(f);
